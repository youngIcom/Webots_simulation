...
void turn180ccw(WbDeviceTag left_motor, WbDeviceTag right_motor) {
  ...
  wb_robot_step(1260);  // Adjust time depending on your robot's speed and turn radius
  
  
  double current_speed = 0.5 * MAX_SPEED;
  double decrement = 0.05 * MAX_SPEED;
  
  while (current_speed > 0.0) {
    current_speed -= decrement;
    if (current_speed < 0.0) current_speed = 0.0;
    
    wb_motor_set_velocity(left_motor, current_speed * -1);
